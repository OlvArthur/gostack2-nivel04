...
let ListProviderDayAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const availability = eachHourArray.map(hour => {
      const availableHour = appointments.find(appointment => (0, _dateFns.getHours)(appointment.date) === hour);
      const currentDate = new Date(Date.now());
      const appointmentDate = new Date(year, month - 1, day, hour);
      return {
        hour,
        available: !availableHour && (0, _dateFns.isAfter)(appointmentDate, currentDate)
      };
    });
    return availability;
  }
